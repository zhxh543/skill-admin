<template>
  <div id="institutionBase">
    <el-button type="primary" @click="modal = true">新增活动基地</el-button>
    ...
    <el-dialog
      ...
      <baseEdit :info="info" @closed="closed"></baseEdit>
    </el-dialog>
  </div>
</template>
...
<script>
  ...
  import baseEdit from '@/components/institution/baseEdit'
  ...
  export default {
    ...
    methods: {
      ...
      deleteRow(id) {
        if (id) {
          this.$confirm('此操作永久删除该基地信息，是否删除?', '提示', {
      ...
      editRow(row) {
        if (row)
          this.$confirm('此基地信息已通过审核，继续操作将重置审核状态，是否继续?', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
